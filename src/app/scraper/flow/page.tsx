...
export default function AutomationPage() {
  ...
  const addStep = () =>
    setSteps((prev) => [...prev, { navigateTo: { url: "" } }]);

  const removeStep = (i: number) => {
    setResults([]);

    const validSteps: flow[] = [];
    for (let j = 0; j < steps.length; j++) {
      ...
      const prevAction = prevStep ? (Object.keys(prevStep)[0] as Action) : null;

      // If this action has rules and prevAction is not allowed, skip it
      if (
        ACTION_RULES[action].length > 0 &&
        (!prevAction || !ACTION_RULES[action].includes(prevAction))
      ) {
        continue;
      }
  ...
  const updateStep = (i: number, action: Action, value: any) =>
    setSteps((prev) =>
      prev.map((s, idx) => (idx === i ? { [action]: value } : s))
    );
  ...
  const runAutomation = async () => {
    ...
    try {
      if (steps[0].navigateTo === undefined)
  ...
  return (
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>
            Are you absolutely sure you want to delete this step?
          </AlertDialogTitle>
          <AlertDialogDescription>
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction onClick={() => onConfirm(...params)}>
            Proceed
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
...
function ActionInputHandler({ action, value, updateStep, idx }: any) {
  // Navigate to
    ...
    return (
      <>
        <Input
          ...
          value={value || ""}
          onChange={(e) =>
            updateStep(idx, "getElementsByXpath", e.target.value)
          }
        />
        <p className="text-sm text-muted-foreground">
          xpath of group of elements e.g.{" "}
  ...
  if (action === "extractText") {
    return (
      <p className="text-sm text-muted-foreground">
        Extracts text from above selected element(s)
      </p>
    );
  }
  ...
  if (action === "extractTable") {
    return (
      <p className="text-sm text-muted-foreground">
        Extracts table from above selected element. Make sure that the element
        is a table. Only works on tables with headers.
      </p>
    );
  }
    ...
    return (
      <>
        <Input
          ...
          value={value || ""}
          onChange={(e) =>
            updateStep(idx, "waitForPageLoad", Number(e.target.value))
          }
        />
        <p className="text-sm text-muted-foreground">
          Waits until page load or timeout (default 5000ms)
        </p>
      </>
    );
  }
    ...
    return (
      <>
        <Input
          ...
          value={value || ""}
          onChange={(e) =>
            updateStep(idx, "waitForFullLoad", Number(e.target.value))
          }
        />
        <p className="text-sm text-muted-foreground">
          Waits for given time to allow full page load (e.g. for infinite scroll
          pages)
        </p>
      </>
    );
  ...
  if (action === "executeJavaScript") {
    return (
      <>
        ...
        />
        <p className="text-sm text-muted-foreground">
          e.g. document.querySelector(&apos;input&apos;).value =
          &apos;text&apos;
        </p>
      </>
    );
  ...
  if (action === "inputText") {
    return (
      <>
        ...
        />
        <p className="text-sm text-muted-foreground">
          Inputs text into above selected element (should be input, textarea or
          [contenteditable])
        </p>
      </>
    );
    ...
    return (
      <>
        <Input
          ...
          value={value || ""}
          onChange={(e) =>
            updateStep(idx, "waitForXpathToDisappear", e.target.value)
          }
        />
        <p className="text-sm text-muted-foreground">
          Waits until the xpath element disappears from page. E.g. for loading
          indicators
        </p>
      </>
    );
    ...
    return (
      <>
        <Input
          ...
          value={value || ""}
          onChange={(e) =>
            updateStep(idx, "waitForCssToDisappear", e.target.value)
          }
        />
        <p className="text-sm text-muted-foreground">
          Waits until the CSS element disappears from page. E.g. for loading
          indicators
        </p>
      </>
    );
  ...
  if (action === "scrollToBottom") {
    return (
      <p className="text-sm text-muted-foreground">Scrolls page to bottom</p>
    );
  }
  if (action === "scrollIntoElement") {
    return (
      <p className="text-sm text-muted-foreground">
        Scrolls into previously selected element
      </p>
    );
  }
  ...
  if (action === "getRequest") {
    return (
      <div className="space-y-2">
        ...
        />
        <p className="text-sm text-muted-foreground">
          Make GET request to given URL with optional headers, etc.
        </p>
      </div>
    );
  }
  ...
  if (action === "postRequest") {
    return (
      <div className="space-y-2">
        ...
        />
        <p className="text-sm text-muted-foreground">
          Make POST request to given URL with optional headers, etc.
        </p>
      </div>
    );
  }
