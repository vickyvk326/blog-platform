    ...
    const requestOptions = {
      ...
      body: JSON.stringify({
        rows: multipleTableInputdata.rows.map((row) =>
          Object.fromEntries(row.map((cell, idx) => [multipleTableInputdata.headers[idx], cell])),
        ),
      }),
    };
        ...
        <TabsContent value='account'>
          <Card>
            <CardContent className='space-y-4 pt-4'>
              ...
              <Select value={type} onValueChange={(val: locatorTypesType) => setType(val)}>
                <SelectTrigger>
        ...
        <TabsContent value='password'>
          <Card>
            <CardContent className='space-y-4 pt-4'>
              ...
              {!!multipleTableInputdata?.headers?.length && !!multipleTableInputdata?.rows?.length && (
                <>
                  <Button onClick={handleMultipleSiteScrape} disabled={loading}>
                    {loading ? 'Scraping...' : 'Scrape Table'}
                  </Button>
                  ...
                  <table className='min-w-full border border-gray-300 rounded-md mt-2'>
                    <thead>
                      <tr className='border-b border-gray-200'>
                        {multipleTableInputdata?.headers.map((head) => (
                          <th key={head} className='px-3 py-2 border-r border-gray-200 font-bold'>
                            {head}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody>
                      {multipleTableInputdata?.rows?.slice(0, 5).map((cells, i) => (
                        <tr key={i} className='border-b border-gray-200'>
                          {cells.map((cell, j) => (
                            <td key={j} className='px-3 py-2 border-r border-gray-200'>
                              {cell}
                            </td>
                          ))}
                        </tr>
                  ...
                  <p className='ps-2.5 font-medium mt-1'>
                    {multipleTableInputdata?.rows.length >= 5 && `${multipleTableInputdata?.rows.length - 5} more rows`}
                  </p>
                </>
              )}
            </CardContent>
          ...
          {result?.type === 'table' &&
            ...
            result.data.map((siteTable, idx) => (
              <div className='overflow-x-auto' key={idx}>
                ...
                <DownloadExcel headers={siteTable?.headers} rows={siteTable?.rows} />
                <table className='min-w-full border border-gray-300 rounded-md mt-2'>
                  <thead>
                    <tr className='border-b border-gray-200'>
                ...
                <p className='ps-2.5 font-medium mt-1'>
                  {siteTable?.rows.length >= 5 && `${siteTable?.rows.length - 5} more rows`}
                </p>
              </div>
            ))}
...
function InputFile({ onUpload }: { onUpload: React.ChangeEventHandler<HTMLInputElement> }) {
  return (
